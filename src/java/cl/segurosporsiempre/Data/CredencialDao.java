...
public class CredencialDao {
    ...
    public Login obtenerCredencial(Long id) {
        ...
        try {
            ...
            CallableStatement cst = conn.getConnection().prepareCall("{ call SP_OBTENER_CREDENCIAL(?, ?)}");
            cst.setLong(1, id);
    ...
    public boolean modificarPersona(Login login) {
        
        Representante r;

        try {
            CallableStatement cst = conn.getConnection().prepareCall("{ call SP_MODIFICAR_CREDENCIAL(?,?,?,?,?,?) }");
            cst.setLong(1, login.getIdLogin());
            cst.setString(2, login.getCorreo()); 
            cst.setLong(3, login.getPerfil().getIdPerfil());        
            cst.setLong(4, login.getCliente().getIdCliete());
            cst.setString(5, login.getRespresentante().getNombre());
            cst.setString(6, login.getRespresentante().getRut());        
    ...
    public boolean modificarClave(Long id, String pass)
    {
         try {
            //TODO: Revisar modificar clave, controlador y vista asociada
            CallableStatement cst = conn.getConnection().prepareCall("{ call SP_MODIFICAR_CLAVE(?,?) }");
            cst.setLong(1, id);
